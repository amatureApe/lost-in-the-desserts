...
contract RareSkillsCTF_LostInTheDesserts {
    error PING();
    error PING2(uint256 x, uint256 y);
    ...
    function le_souffle(
        ...
        uint256 eclair2,
        uint256 meringue2
    ) public view returns (uint256 eclair, uint256 meringue) {
        (bool ok, bytes memory result) = address(6).staticcall(
            abi.encode(eclair1, meringue1, eclair2, meringue2)
        );
        require(ok, "x failed");
        (eclair, meringue) = abi.decode(result, (uint256, uint256));
    }

    function les_macarons(
        uint256 pain_au_chocolat,
        uint256 eclair1,
        uint256 meringue1
    ) public view returns (uint256 eclair, uint256 meringue) {
        (bool ok, bytes memory result) = address(7).staticcall(
            abi.encode(eclair1, meringue1, pain_au_chocolat)
        );
        require(ok, "y failed");
        (eclair, meringue) = abi.decode(result, (uint256, uint256));
    }
    ...
    function pralines(
        ...
        uint256 c,
        uint256 d,
        uint256[4] calldata flaugnarde
    ) public view returns (uint256 u) {
        ...
        (bool ok, bytes memory result) = address(8).staticcall(
            abi.encode(a, b, clafoutis, c, d, clafoutis, 1, 2, flaugnarde)
        );
